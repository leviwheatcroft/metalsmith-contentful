...
export default class Cache {
  ...
  upsertCollection (docs) {
    return vow.all(docs.map((doc) => this.upsert(doc)))
  }
  ...
  resolve (doc, depth) {
    ...
    let resolvers = []
    const isLink = function (object) {
      return object && object.sys && object.sys.type === 'Link'
    }
    Object.keys(doc).forEach((key) => {
      if (isLink(doc[key])) {
        resolvers.push(
          this.findOne({'sys.id': doc[key].sys.id}, depth - 1).then((child) => {
